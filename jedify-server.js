...
function buildDefaultTemplate() {
  return {
    ...
    isDefault: true,
    slides: [
      { title: 'Title', description: 'Dark full-bleed slide. Top-left: "RUBYPLAY × [OPERATOR NAME]" in small caps. Red left-border accent bar. QBR badge (red pill: "Q[N] [YEAR] · QUARTERLY BUSINESS REVIEW"). Main headline: the single strongest, most surprising fact from the data — a number, a percentage, or a bold claim (e.g. "GGR Grew +430% in 90 Days"). Below headline: one-line sub-headline in red explaining the strategic implication. Date range line: "[Operator] · [Month] – [Month] [Year] · [Market] · via RubyPlay". Bottom row: 4 KPI stat boxes (dark cards) showing the most important numbers from the data — e.g. Total Bets, GGR, Active Players, and one performance highlight. Each box: small label in caps, large red number, small context note. RubyPlay logo bottom-left. Copyright bottom-right.' },
      { title: 'KPI Charts', description: '2×2 grid of SVG bar charts. Each chart covers 6 months. Charts: (1) Total Bets (€), (2) GGR (€), (3) Active Players, (4) Rounds per Player. For each chart: QBR month bar is red (#CC0000), all prior months are dark grey (#3A3A3A). Label the value on top of every bar. Show the % change vs prior month below each chart title (green if positive, red if negative). Chart titles in white caps. Grid lines subtle. Dark card background per chart. Headline above the grid: one sentence summarising the most important trend across all 4 KPIs.' },
      { title: 'Studio Summary Table', description: 'Full-width table. Columns: Studio | Active Games | Total Bets (€) | Bet Share % | Total GGR (€) | GGR/Game. Sort by Total Bets desc. Highlight the top studio row with a red left border and slightly lighter background. Show Bet Share % as a coloured progress bar inside the cell. Add a "LEADER" badge on the top row. Footer row: Totals. Headline: which studio dominates and what that means for the operator.' },
      { title: 'Studio Performance', description: 'Full-width SVG multi-line chart. One coloured line per studio over the last 6 months (x-axis = months, y-axis = Total Bets €). Each line a distinct colour; label each line at its endpoint. Mark the current QBR month with a vertical dashed red line. Legend below chart. Headline: which studio is growing fastest or diverging from others. If only one studio exists, show a single-line chart with data points marked.' },
      { title: 'New Games Launched', description: 'Table of games launched in the last 30–60 days. Columns: Game | Studio | Launch Date | RTP % | Total Bets (€) 14d | Active Players 14d | Trend (↑↓→). Sort by Total Bets desc. Highlight the top performer row in red. Add a "TOP LAUNCH" badge on the best game. Add a "UNDERPERFORMING" grey badge on any game with very low bets relative to others. Below table: one insight sentence about which launch is succeeding and why it matters. Headline: strongest launch performance fact.' },
      ...
      { title: 'VIP Analysis', description: 'ALWAYS write one of these two sentences first as a bold status line: either "VIP player segmentation data is available" or "No VIP player segmentation data available for this operator." If player-level VIP data is available: Table: Game | Studio | VIP Players | VIP Bets (€) | VIP GGR (€) | VIP GGR/Player, sorted by VIP Bets desc. Highlight top VIP game in red. Add insight: what % of total GGR comes from VIP players. If no VIP segmentation data: state this clearly, show the Top 10 Players by total bets as a proxy table (Player ID | Total Bets | GGR | Games Played), and add an action card: "Enable VIP player segmentation to unlock this analysis." Headline: most important VIP insight or the gap it represents.' },
      { title: 'Max Bet Analysis',    description: 'PLACEHOLDER — Coming Soon.' },
      { title: 'Promotion Analysis',  description: 'PLACEHOLDER — Coming Soon.' },
      { title: 'The Portfolio Gap', description: 'Two-section layout. TOP: Market benchmark table — columns: Market Rank | Game | Studio | In Operator Top 10? | Operator Rank | Operator Bets (€) | Status. Show top 5–8 market games. "MISSING" rows highlighted in red with ✗ icon; present games show ✓ in green. BOTTOM: Two side-by-side tier boxes. TIER 1 (red) = Critical: games missing entirely or severely underperforming vs market rank. TIER 2 (amber) = High-value: games present but underexposed. Each tier: small table with Game | Market Rank | Operator Bets | Opportunity note. Below tier boxes: one red callout card with the single biggest revenue opportunity (e.g. "2% market share on [Game] = +€X quarterly"). Headline: the most striking gap between market reality and operator portfolio.' },
      { title: 'Growth Levers', description: 'Table of games with high GGR/Player but low player reach — the hidden high-performers. Columns: Game | Studio | GGR/Player (€) | Active Players | % of Total Players | Opportunity | Action. Sort by GGR/Player desc. Action column: red "EXPAND" pill for games already live with low reach; blue "PROMOTE" for games with very few players. Below table: 3 KPI stat cards — Highest GGR/Player, Portfolio Median GGR/Player (with multiplier vs highest), Players Yet to Discover These Games. Headline: frame as the untapped revenue hiding inside the current portfolio.' },
      { title: 'KPI Gaps', description: 'Benchmark comparison table. Columns: KPI | Operator Value | Market Benchmark | Gap | Signal. KPIs to include: GGR/Active Player, Rounds/Player, Active Player Rate, New Game Adoption Rate, Top-10 Game Concentration. Gap column: red for negative gaps (operator below benchmark), green for positive. Signal column: coloured badge — "CRITICAL", "IMPROVE", "ON TRACK", "LEADING". Below table: 2–3 sentence narrative explaining the most important gap and its business impact. Headline: the KPI gap that most limits growth.' },
      { title: 'Actions & Priorities', description: 'Up to 5 numbered action cards, each on its own row. Each card: number badge (red circle), priority label pill (CRITICAL / HIGH / MEDIUM in matching colour), bold action title, one-line rationale from the data, expected outcome or uplift estimate. Sort by priority. Actions should be specific and actionable (e.g. "Activate Loco Habanero — #1 in Mexico with €0 at Codere" not "Add missing games"). Footer: "Agreed next step: [date] follow-up with [specific milestone]." Headline: "Here Is What We Do Next."' },
      { title: 'The Ask', description: 'Dark full-bleed closing slide. Red left accent bar. Large white headline (the single specific ask — one sentence, bold, direct). Below: 3 bullet next steps with target dates, each starting with an action verb. Bottom section: two side-by-side cards — LEFT: "What We Commit" (RubyPlay deliverables), RIGHT: "What We Need" (operator actions). RubyPlay logo bottom-left. Headline must be a direct ask, not a topic — e.g. "Activate 3 missing top-10 games before end of Q2" not "Next Steps."' },
    ],
    brand: {
