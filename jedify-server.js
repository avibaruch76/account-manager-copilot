...
function buildDefaultTemplate() {
  return {
    ...
    slides: [
      ...
      { title: 'Player Segmentation', description: 'Two side-by-side SVG stacked bar charts. LEFT: monthly stacked bar chart — X axis = months, Y axis = player count. Each bar stacked by player segment (e.g. New Players, Returning, Dormant Reactivated, High Value, etc.). "New Players" segment MUST always appear as one of the stacks. Each segment gets a distinct color with a shared legend below. RIGHT: identical chart but Y axis = % share (each bar totals 100%) — shows how the mix of segments shifts month over month. Below both charts: 2–3 sentence insight on trend — is the new player ratio growing or shrinking, which segment is driving volume changes.' },
