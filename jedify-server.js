...
function buildDefaultTemplate() {
  return {
    ...
    slides: [
      ...
      { title: 'VIP Analysis',        description: 'ALWAYS write one of these two sentences first: either "VIP player segmentation data is available" or "No VIP player segmentation data available for this operator." If data is available, show Table: Game | Studio | VIP Players | VIP Bets (€) | VIP GGR (€), sorted by VIP Bets desc. If no VIP player segmentation data exists, note it clearly, show the top 10 players and add it as an action item.' },
      ...
      { title: 'KPI Gaps',            description: 'Two-column layout. LEFT (70%): Table with columns KPI | Our Value | Market Benchmark (Range) | Gap | Trend. Gap column uses colored status pills — e.g. "ABOVE AVERAGE – 2ND BEST" (green), "MIDDLE OF RANGE" (grey), "BELOW LEADERS — 47% GAP" (red). Below the table: SVG horizontal bar chart for the single most important KPI gap, showing Our Value vs Market Leader (two bars, labeled, different colors). RIGHT (30%): "Root Causes" bullet list (3 bullets with colored dots) + a highlighted "The Fix Is Clear" box with 2-3 sentences. Trend arrows: green up-arrow for positive trend, red right-arrow for flat/negative.' },
